...
exports.convertSectionId = (sectionId, title) => {
    ...
    if (title !== undefined) {
        ...
        convertedSectionId = sectionId.replace(new RegExp('/@', 'g'), '::');
    }
...
exports.buildColumnControlId = (baseId, idBreadcrumbs, breadcrumbs, controlType, title) => {
        ...
        if (item === 'sections') {
            for (let index = 0; index < breadcrumbs.length; index++) {
                ...
                if (breadcrumb === 'sections') {
                    const sectionId = breadcrumbs[index + 1];
    ...
    if (breadcrumbs[0] === 'sections') {
        tableId = breadcrumbs[1].replace(/::/g, ':3a:3a');
    }
    const lastIdPart = breadcrumbs[breadcrumbs.length - 1].replace(/[/]/g, '_');
    let fullId;
