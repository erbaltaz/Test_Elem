...
function getSettings(routing) {
    const flexibleColumnLayout = !!(routing.config && routing.config.routerClass === v4_1.FIORI_FCL_ROUTER_CLASS);
    return flexibleColumnLayout ? { flexibleColumnLayout } : undefined;
}
class V4AppProvider extends appProvider_1.AppProvider {
    constructor(manifest, schemas) {
        super(manifest, schemas, getHome(manifest['sap.ui5'].routing), getPages(manifest, schemas), common_1.FioriElementsVersion.v4, getSettings(manifest['sap.ui5'].routing));
