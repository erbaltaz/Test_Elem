import { AppSettings as CommonSettingsV4 } from '../../specification/v4/ApplicationV4';
import { Application } from '../../specification/common/Application';
import { Pages, Target } from '../../specification/common';
export declare const getSettingsPathForPage: (pageName: string[]) => string;
export declare const getControlConfigurationPath: (pageName: string[]) => string;
export declare const getLineItemPathForLR: (pageName: string[]) => string;
export declare const getTableSettingsPathForLR: (pageName: string[]) => string;
export declare const getTableSettingsPathForOP: (pageName: string[], sectionId: string) => string;
export declare const getCustomSectionPath: (pageName: string[], ...pathParts: string[]) => string;
export declare class ApplicationV4 implements Application {
    $schema?: string;
    id?: string;
    pages?: Pages;
    home?: string;
    target?: Target;
    settings?: AppSettings;
}
export declare class AppSettings implements CommonSettingsV4 {
    title?: string;
    description?: string;
    flexibleColumnLayout?: boolean;
}
