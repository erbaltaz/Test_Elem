...
const applyFCLRouting = (manifestUI5, app, pageName, targetPages = [], level = 0) => {
    ...
    if (page.navigation) {
        ...
        if (level === 2) {
            // For third level pages apply 'ThreeColumnsEndExpanded' layout as default
            // It was requested for Demo
            if (target.options) {
                const navigations = target.options.settings.navigation || {};
                for (const navigation in navigations) {
                    navigations[navigation].detail.layout = v4_1.FlexibleColumnLayoutType.ThreeColumnsEndExpanded;
                }
            }
        }
    }
