...
function transformRoutingV4(homePage, pages, manifest) {
    ...
    for (const id in pages) {
        let target;
        const page = pages[id];
        if (page.pageType !== v4_1.PageType.CustomPage) {
            target = {
                type: 'Component',
                id,
        ...
        if (page.navigation) {
            for (const association in page.navigation) {
                ...
                parts.forEach((part, index) => {
                    let key = '';
                    if (pageType !== v4_1.PageType.ListReport) {
                ...
                if (pageType === v4_1.PageType.ListReport ||
                    (pageType === v4_1.PageType.CustomPage && !pages[association].entitySet && !navigationKey)) {
