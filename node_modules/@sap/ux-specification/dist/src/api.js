...
function exportConfig(exportParameters) {
    ...
    if (exportParameters[page_1.FioriElementsVersion.v2]) {
        ...
        if (v4Params[schemaAccess_1.SchemaType.ListReport]) {
            const { appId, page, manifest, jsonSchema } = v4Params[schemaAccess_1.SchemaType.ListReport];
            exportResults = export_2.exportListReportPage(appId, page.config, manifest, [page.name], jsonSchema);
        }
        else if (v4Params[schemaAccess_1.SchemaType.ObjectPage]) {
            const { appId, page, manifest, jsonSchema } = v4Params[schemaAccess_1.SchemaType.ObjectPage];
            exportResults = export_2.exportObjectPage(appId, page.config, manifest, [page.name], jsonSchema);
        }
        else if (v4Params[schemaAccess_1.SchemaType.Application]) {
            const { application, manifest, jsonSchema } = v4Params[schemaAccess_1.SchemaType.Application];
            exportResults = export_2.exportApplicationV4(application, manifest, jsonSchema);
        }
    }
